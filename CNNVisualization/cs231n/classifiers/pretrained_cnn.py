...
class PretrainedCNN(object):
  ...
  def forward(self, X, start=None, end=None, mode='test', idxFilter = None):
    """
    ...
    for i in xrange(start, end + 1):
      i1 = i + 1
      if i == end and idxFilter is not None:
        w, b = self.params['W%d'%i1][idxFilter], self.params['b%d'%i1][idxFilter]
        gamma, beta= self.params['gamma%d' % i1][idxFilter], self.params['beta%d' % i1][idxFilter]
    ...
    out = prev_a
    if idxFilter is None:
      cache = (start, end, layer_caches, -1)
    else:
      cache = (start, end, layer_caches, idxFilter)
    return out, cache
  ...
  def backward(self, dout, cache):
    ...
    """
    start, end, layer_caches, idxFilter = cache
    dnext_a = dout
    grads = {}
